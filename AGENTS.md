...
ZenVis Backend 的 AI Agent 基于 Spring AI、MCP 工具调用和 Skill 配置构建。当前巡检智能体不生成 SQL、不执行数据库查询，也不生成 ECharts JSON；巡检过程只通过只读 retrieval MCP 工具获取数据并输出文本或 Markdown 分析。

## 核心 Agent

### InspectionAgent（智能巡检 Agent）
...
RAG 相关能力位于 `com.coolxer.service.dih.rag`，服务于普通聊天的向量检索增强。插件安装后会把 `00_doc` 文档加载到 RAG，可通过 `VectorStoreQueryController` 管理这些插件文档。巡检智能体不维护单独的数据表结构向量召回链路。

## 开发注意事项

- 巡检 Agent 新能力应优先通过 retrieval MCP 工具扩展，而不是在 Agent 内直接访问数据库。
- 不要恢复 SQL 生成、SQL 执行、SQL 校验或图表自动生成链路。
- 新增工具时需要在 `AgentMcpToolService` 中明确 agent 可见范围。
- 会产生副作用的工具必须避免暴露给 `agent_inspect`。
