...
| copyable | boolean | 否 | 字段值是否支持复制（默认值：false） |
| mapping | object | 否 | 映射关系 |
| display_type | string | 否 | 显示类型（默认值：string） |
| search_type | string | 否 | 检索输入类型：date（日期）、datetime（日期时间）、number（数字）、string（普通输入，默认） |
