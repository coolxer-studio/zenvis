...
- 普通数据接入和数据可视化请求由共享工作流维护会话状态、严格候选、MCP 证据、
  锁定参数、失败账本和精确动作协议；内置示例在模型校验前进入确定性演示路由；
- 数据可视化 Agent 不接入外部 MCP，也不执行任意 SQL 或实体写入；先查询实体与字段 Meta，
  再使用本地实体分析白名单；配置、看板和菜单的受控写入默认进入审批并在创建后读回；
