...
- 普通数据接入和数据可视化卡片通过 `WorkflowOrchestrator` 装饰，前端使用
  `/chat/workflow/action` 精确提交，不得回退修改最近同类型卡片；
- MCP 返回的实体和字段选项必须使用 `strictOptions` 并保存服务端候选快照，不能允许自由输入；
- 数据可视化 Agent 不直接访问数据库、不生成任意 SQL、不加载外部 MCP；变更其工具白名单时
  必须区分查询工具与受控配置/看板/菜单写工具，并为写操作保留 `ASK/HIGH` 审批；
- 对话和图表库共同使用 `SafeEcharts`；图表记录必须优先保存真实 `echartsOption` 快照，
  刷新只能调用固定实体分析接口，失败不能覆盖旧快照；
- 内置演示的默认 `ASK` 写工具必须逐次展示真实 MCP 审批卡，不能被全局 `ALLOW`
  覆盖或会话授权跳过；全局 `DENY` 仍优先。

详细设计见 [AI 与数据智能](../07-AI与数据智能/README.md)和
[DIH 数据接入与数据可视化工作流](../07-AI与数据智能/DIH数据接入与可视化工作流.md)。
