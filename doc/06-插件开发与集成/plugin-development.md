...
- 插件升级只执行尚未执行的迁移，已执行迁移的文件名、版本和 SHA-256 必须保持不变。迁移必须向前兼容旧版本，以便升级失败时旧运行版本可以恢复。
- 插件卸载不会回滚 MySQL 迁移，也不会删除迁移历史；如需清理插件 MySQL 业务表，应提供独立、显式的数据治理流程。

### 版本升级兼容规则

- `index.json` 的 `version` 必须是合法 SemVer 2.0.0；升级包版本必须严格高于当前版本，且 `package_name` 必须完全一致。
- ClickHouse 首版升级只允许新增实体、表和字段。删除或重命名实体/字段，以及修改表名、列名、字段类型、数据源、引擎、排序键或分区键都会在维护窗口前被拒绝。
- MySQL 迁移只向前执行且不自动回滚；新增迁移版本必须高于已经执行的最高版本。
- UI、Meta、Skill、RAG 和任务定义以新包为准。看板 `code`、MCP `code`、菜单匹配键和任务名称应保持稳定，平台据此保留数据库 ID、默认状态、用户连接参数、角色授权和任务启停状态。
- 升级期间插件 API 和推送任务会短暂停止。失败时平台恢复旧目录与运行配置，但已执行的 MySQL 迁移以及新增的 ClickHouse 表/字段不会删除。
...
升级走非破坏性路径，保留 ClickHouse/MySQL 业务数据和迁移历史；卸载会删除插件 Meta 关联的 ClickHouse 表和数据，操作前必须备份。MySQL 迁移及其历史不会由卸载流程自动回滚。
