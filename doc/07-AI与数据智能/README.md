...
加载数据接入 Skill，帮助理解数据字典、规划实体和推送任务，并使用 scope 允许的 MCP 工具。

### 数据可视化

由统一 Prompt Runtime 驱动，不直接访问数据库、不生成任意 SQL，也不加载外部 MCP。当前本地白名单包括 Retrieval、实体列表/详情/统计/趋势，以及可视化工作流需要的配置读取/新增/应用、看板创建/查看和菜单创建/查看工具。写入类工具声明为高风险并在执行前进入审批，删除、实体更新和未列入白名单的工具不会注入。

所有业务 Agent 都不检索公共 RAG；请求中的 `deep_think=true` 对 Agent 会被忽略。只有 `type=ask` 允许 RAG 和深度思考，且始终不获得 MCP 工具或 Skill。

### 研判与策略控制

研判 Agent 组织证据、分析链和结论。策略控制涉及写入或执行时，必须通过 MCP 工具策略和审批。
