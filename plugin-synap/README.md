...
`com.coolxer.plugin.synap` 1.1.2 是面向 Zenvis 首次安装和升级场景的探针 Kafka 数据采集插件。它消费 Android、H5、iOS、Host 和 WeChat 探针事实主题，将统一消息信封转换为 `synap_agent_message` 实体，并支持 Host AuditData、WeChat 新事实、传输上下文与风险评估的结构化检索。

## 主要能力

- 按正则订阅探针 Kafka 事实主题，新增同前缀主题无需修改配置。
- 对 JSON 信封、必填字段、时间、整数、经纬度、识别标签和处置结构执行确定性校验。
- 自动创建 `zenvis.synap_agent_message` ClickHouse 表。
...
- 提供单条详情和无菜单的参数聚合分析页，可按设备 ID、IP 地址或启动 ID 查看命中总量，并在“分布统计 / 趋势图 / 时间轴”主 Tab 中分析分类构成与时序；不安装消息管理应用、插件菜单或数据看板。
...
Kafka 主题
  ...
  └── ^(android|h5|ios|host|wechat)_fact_.*$
        ▼
      Vector JSON 解析与业务校验
        ├── 正常 ──▶ zenvis.synap_agent_message
        └── 异常 ──▶ com.coolxer.plugin.synap.dead-letter
```

上游已经是 Kafka，因此插件只安装一项 Kafka → ClickHouse 推送任务，不创建重复的 source-to-Kafka 服务。

主题过滤发生在 Kafka Source 订阅阶段。不匹配 `SYNAP_KAFKA_TOPIC_PATTERN` 的主题不会进入转换器，因此不会写入 ClickHouse 或 DLQ。匹配主题中的消息只有在解析或校验失败时才进入 DLQ。插件不限制 `fact.type` 枚举，只要求其非空且整条消息满足结构契约。
...
| 参数聚合页面 | `com.coolxer.plugin.synap.parameter-analytics`（支持设备 ID、IP、启动 ID，不注册菜单） |
| 消息管理应用 | 不提供 |
...
| `SYNAP_KAFKA_GROUP_ID` | `zenvis-plugin-synap-agent-message-v2` | 消费组 |
| `SYNAP_KAFKA_TOPIC_PATTERN` | `^(android\|h5\|ios\|host\|wechat)_fact_.*$` | 探针主题正则 |
...
任何确定性解析或校验失败都会从正常输出重路由到 `${SYNAP_DLQ_TOPIC}`。DLQ 使用 JSON 编码，保留：
...
DLQ 由 Kafka 运维侧负责保留策略。修复数据后，应将原始 `message` 重放到原始 `topic`；不要把 DLQ JSON 信封直接发送到源主题，也不要把 DLQ 主题纳入 `SYNAP_KAFKA_TOPIC_PATTERN`。

## 安装

1. 在本仓库执行 `./build.sh`。
2. 在 Zenvis 的“插件管理”上传 `com-coolxer-plugin-synap.tar.gz`。
3. 安装插件。平台会自动创建 Meta、ClickHouse 表、推送任务和独立低代码页面；插件不注册菜单或看板。
4. 在“数据推送服务”确认“探针 Kafka 消息入库”已启动。
5. 通过平台实体检索检查记录；消息 ID 链接进入记录详情页，设备 ID、IP 与启动 ID 字段链接进入参数值聚合页。
...
- `guid` 使用 `parameter-analytics?guid=` 打开聚合页并按设备 ID 精确计数；`lan_ip`、`wan_ip` 使用 `parameter-analytics?ip=` 打开同一页面并在两个 IP 字段间 OR 计数；`start_id` 使用 `parameter-analytics?start_id=` 按启动 ID 计数。链接进入后会自动查询，默认统计近 7 天，支持切换近 24 小时和近 30 天。
- `message_id` 使用当前行隐藏返回的 `zenvis_id` 打开 `detail-message`，详情接口仍按平台记录 ID 精确读取；未提供消息 ID 的记录保持空单元格，不显示可点击入口。
- 参数聚合页调用值统计、趋势、检索和分布接口，主 Tab 依次为“分布统计”“趋势图”“时间轴”，默认首先展示分布统计。页面从设备 ID、IP 或启动 ID 链接进入以及浏览器直接刷新时都会自动查询；同时存在多个 URL 参数时按设备 ID、启动 ID、IP 的顺序选择。趋势图直接使用趋势接口的数据作用域，时间轴和分布统计分别读取最近 100 条明细，避免嵌套服务覆盖趋势数据。时间轴只列出命中数大于零的分桶，并汇总数据类别、识别标签和处置类型；节点命中数来自全量趋势聚合。
