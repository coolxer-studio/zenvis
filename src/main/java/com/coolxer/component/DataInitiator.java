...
public class DataInitiator {
    ...
    private void initDefaultPermission() {
        if (CollectionUtils.isEmpty(menuRepository.findAll())) {
            ...
            menuRepository.save(new Menu().setName("VectorStore 管理").setType(MenuType.LOW_CODE_PAGE).setRoute(MenuType.LOW_CODE_PAGE.getRoute()).setParams("vectorstore").setIsEditable(false).setParentId(serviceMenu.getId()).setOrderNumber(5).setLevel(MenuLevel.LEVEL_2));
