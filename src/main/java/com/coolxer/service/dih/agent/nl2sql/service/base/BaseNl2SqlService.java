...
import com.coolxer.service.dih.agent.nl2sql.util.DateTimeUtil;
import com.coolxer.service.dih.agent.nl2sql.util.MarkdownParser;
...
import static com.coolxer.service.dih.agent.nl2sql.constant.Constant.INTENT_UNCLEAR;
import static com.coolxer.service.dih.agent.nl2sql.constant.Constant.SMALL_TALK_REJECT;
import static com.coolxer.service.dih.agent.prompt.PromptConstant.getQuestionExpansionPromptTemplate;
import static com.coolxer.service.dih.agent.prompt.PromptHelper.buildMixMacSqlDbPrompt;
import static com.coolxer.service.dih.agent.prompt.PromptHelper.buildMixSelectorPrompt;
...
public class BaseNl2SqlService {
	...
	/**
	 * 带对话历史的 query rewrite
	 * @param query 用户当前输入
	 * @param conversationHistory 对话历史（可为 null）
	 */
	public String rewrite(String query, List<Message> conversationHistory) throws Exception {
		...
		String prompt = PromptHelper.buildRewritePrompt(query, schemaDTO, evidences, conversationHistory);
